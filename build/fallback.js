...
var drawBucketChart = function (withCategory) {
  Highcharts.chart('bucketchart', {
    ...
    series: [{
      ...
      data: [{
        name: '< 1000',
        y: -1 * withCategory.filter(x => x.amount >= -1000 && x.amount < 0).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }, {
        name: '< 5000',
        y: -1 * withCategory.filter(x => x.amount >= -5000 && x.amount < -1000).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }, {
        name: '< 10000',
        y: -1 * withCategory.filter(x => x.amount >= -10000 && x.amount < -5000).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }, {
        name: '< 50000',
        y: -1 * withCategory.filter(x => x.amount >= -50000 && x.amount < -10000).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }, {
        name: '< 100000',
        y: -1 * withCategory.filter(x => x.amount >= -100000 && x.amount < -50000).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }, {
        name: '> 100000',
        y: -1 * withCategory.filter(x => x.amount < -100000).reduce((prev, curr) => prev + Math.round(curr.amount), 0),
      }]
    }]
  });
};
